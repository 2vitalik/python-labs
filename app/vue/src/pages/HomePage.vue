...
<template>
  <div>
    ...
    <template v-if="home">
      <GuideText :text="text" />
      <div class="more small mt-4 text-end">
        <div><RouterLink to="/changes">{{ pages.changes?.title || 'Що змінилось' }}</RouterLink></div>
        <div class="mt-1"><RouterLink to="/guide">📖 Уся методичка</RouterLink></div>
      </div>
    </template>
  </div>
</template>
...
<style scoped>
.more a { color: var(--bs-secondary-color); text-decoration: none; }
