...
- ✅ **Бекенд «Моєї гри» v1** (2026-08-16, [T91](.t/T91--my-game-backend/report.md), дизайн — [T90](.t/T90-P--my-game-v1-design.md)): `/api/works` — гра студента (одна, unique owner), обʼєкти вікно/меню (авто-заявка типу, каскадне видалення), заявки з дедупом, скриншоти (`uploads/`, 8 МБ, StaticFiles); смоук 35/35 — `app/api/tests/smoke_works.py`; фронт — етап C
...
- **«Моя гра»** ([T90](.t/T90-P--my-game-v1-design.md)): `works` (одна на студента, основа — слаг гри або свій текст), `work_objects` (вікно: `task`-тип з тегом `window`, скриншоти; меню: `window`-хост + `items` `{title, window, task, note}` — обидві цілі порожні = текстовий пункт), `claims` (`task`+`object`, унікальність трійки, `status=claimed` — життєвий цикл виросте з [T10](../reports/.t/T10-Q--reports-open-questions.md)); файли — `app/api/uploads/<work>/<uuid>` (git-ignored); видалення логуються `record_delete()`
- Ескіз майбутніх колекцій (reports, defenses, remarks, coins-журнал, surveys…) — [T59](.t/T59-C--data-model-sketch.md); зараз існують `users` + `history` + `games` + `tasks` + `works` + `work_objects` + `claims`
...
- **Пілот «Моя гра»**: бекенд готовий ([T91](.t/T91--my-game-backend/report.md)) → етап C: фронт (ескіз — [T90](.t/T90-P--my-game-v1-design.md)) — після відповідей [T92](.t/T92-Q--naming-urls.md) (імена моделей `Work`/`WorkObject`/`Claim` і URL-схема сторінок; перейменування зараз без міграцій — колекції порожні); Vitalik: відповіді T92 + активувати `window`-картки після ревізії (інакше студентам лише 4 типи вікон).
