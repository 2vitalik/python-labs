...
- **Пілот «Моя гра»**: бекенд готовий ([T91](.t/T91--my-game-backend/report.md)) → етап C: фронт `/my-game` + `/works` + `/works/:id` (ескіз — [T90](.t/T90-P--my-game-v1-design.md)); Vitalik: активувати `window`-картки після ревізії (інакше студентам лише 4 типи вікон).
