...
- **Пілот «Моя гра»: етапи A–C виконано** ([T91](.t/T91--my-game-backend/report.md) + [T93](.t/T93--my-game-front/report.md)) → Vitalik: перезапуск API + клік-тест `/my/game` → `/students` → паспорт; активувати `window`-картки після ревізії (інакше студентам лише 4 типи вікон). Далі етап D: граф переходів на паспорті; сесія механік (T86 Q1c) — окремо.
